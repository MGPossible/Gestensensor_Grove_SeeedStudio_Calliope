// main.ts

const initRegisterArray: number[] = [
    
]

enum GroveGesture {
    //% block="0 - Keine"
    None = 0,
    //% block="1 - Rechts"
    Right = 1,
    //% block="2 - Links"
    Left = 2,
    //% block="3 - Oben"
    Up = 3,
    //% block="4 - Unten"
    Down = 4,
    //% block="5 - Vorwärts"
    Forward = 5,
    //% block="6 - Rückwärts"
    Backward = 6,
    //% block="7 - Uhr (Clockwise)"
    Clockwise = 7,
    //% block="8 - Gegen Uhr (Anticlockwise)"
    Anticlockwise = 8,
    //% block="9 - Winken"
    Wave = 9
}

/**
 * Grove Gesture Sensor Erweiterung
 */
//% color=#8f3fd1 icon="\uf20e" block="Gestensensor MG"
//% groups=['Grundfunktionen', 'Erweiterungen']
namespace gesten {

    const PAJ7620_ADDR = 0x73
    const PAJ7620_ID_ADDR = 0x00
    const gestureEventId = 3100

    // PAJ7620 Sensor-Klasse
    class PAJ7620 {
        private paj7620WriteReg(addr: number, cmd: number) {
            let buf = pins.createBuffer(2)
            buf[0] = addr
            buf[1] = cmd
            pins.i2cWriteBuffer(PAJ7620_ADDR, buf, false)
        }

        private paj7620ReadReg(addr: number): number {
            let buf = pins.createBuffer(1)
            buf[0] = addr
            pins.i2cWriteBuffer(PAJ7620_ADDR, buf, false)
            buf = pins.i2cReadBuffer(PAJ7620_ADDR, 1, false)
            return buf[0]
        }

        private paj7620SelectBank(bank: number) {
            if (bank === 0) this.paj7620WriteReg(0xEF, 0)
            else if (bank === 1) this.paj7620WriteReg(0xEF, 1)
        }

        private paj7620Init() {
            let temp = 0
            this.paj7620SelectBank(0)
            temp = this.paj7620ReadReg(PAJ7620_ID_ADDR)
            if (temp === 0x20) {
                for (let i = 0; i < 438; i += 2) {
                    this.paj7620WriteReg(initRegisterArray[i], initRegisterArray[i + 1])
                }
            }
            this.paj7620SelectBank(0)
        }

        init() {
            this.paj7620Init()
            basic.pause(200)
        }

        read(): number {
            let data = 0
            let result = 0

            data = this.paj7620ReadReg(0x43)
            switch (data) {
                case 0x01:
                    result = GroveGesture.Right
                    break
                case 0x02:
                    result = GroveGesture.Left
                    break
                case 0x04:
                    result = GroveGesture.Up
                    break
                case 0x08:
                    result = GroveGesture.Down
                    break
                case 0x10:
                    result = GroveGesture.Forward
                    break
                case 0x20:
                    result = GroveGesture.Backward
                    break
                case 0x40:
                    result = GroveGesture.Clockwise
                    break
                case 0x80:
                    result = GroveGesture.Anticlockwise
                    break
                default:
                    data = this.paj7620ReadReg(0x44)
                    if (data === 0x01) {
                        result = GroveGesture.Wave
                    }
                    break
            }
            return result
        }
    }

    // Globale Variablen
    let paj7620 = new PAJ7620()
    let sensorInitialisiert = false
    let letzteGeste = 0
    let triggeredGestures: boolean[] = [false, false, false, false, false, false, false, false, false, false]
    let monitoringLaeuft = false

    // NEU: Cache für das Clear-on-Read Problem
    let aktuelleGesteCache = 0
    let letzteGesteZeit = 0

    // Zentrale Überwachungsschleife (läuft nur EINMAL)
    function startGestureMonitoring() {
        if (monitoringLaeuft) return
        
        monitoringLaeuft = true
        control.inBackground(() => {
            while (true) {
                if (sensorInitialisiert) {
                    // Wir nutzen jetzt unsere smarte Cache-Lese-Funktion
                    const aktuelleGeste = gestureLesen() 
                    
                    // Nur wenn sich Geste ändert
                    if (aktuelleGeste !== letzteGeste) {
                        letzteGeste = aktuelleGeste
                        
                        // Events feuern wir nur ab, wenn es keine "0" ist
                        if (aktuelleGeste !== 0) {
                            control.raiseEvent(gestureEventId, aktuelleGeste)
                        }
                    }
                }
                basic.pause(50)
            }
        })
    }

    /**
     * Startet den Gestensensor an A0 und bereitet ihn für die Verwendung vor.
     */
    //% group="Grundfunktionen"
    //% block="Gestensensor an A0 initialisieren"
    //% block.tooltip="Startet den Gestensensor an A0 und bereitet ihn für die Verwendung vor."
    export function initialisieren(): void {
        basic.pause(100)
        paj7620.init()
        sensorInitialisiert = true
        basic.pause(200)
        
        // Starte zentrale Überwachung
        startGestureMonitoring()
    }

    /**
     * Liest die aktuelle Geste vom Sensor (0=Keine, 1=Rechts, 2=Links, 3=Oben, 4=Unten, 5=Vorwärts, 6=Rückwärts, 7=Uhr, 8=Gegen Uhr, 9=Winken).
     */
    //% group="Grundfunktionen"
    //% block="Geste erkennen (Zahlwert)"
    //% block.tooltip="Liest die aktuelle Geste vom Sensor (0=Keine, 1=Rechts, 2=Links, 3=Oben, 4=Unten, 5=Vorwärts, 6=Rückwärts, 7=Uhr, 8=Gegen Uhr, 9=Winken)"
    export function gestureLesen(): number {
        if (!sensorInitialisiert) {
            return 0
        }

        // Immer echten Wert abfragen
        let gelesen = paj7620.read()
        let zeit = input.runningTime()

        // Wenn der Sensor eine aktive Bewegung erfasst hat, Cache erneuern
        if (gelesen !== 0) {
            aktuelleGesteCache = gelesen
            letzteGesteZeit = zeit
        }

        if (zeit - letzteGesteZeit < 150) {
            return aktuelleGesteCache
        } else {
            return 0
        }
    }

    /**
     * Gibt "WAHR" zurück wenn die angegebene Geste erkannt wurde.
     */
    //% group="Grundfunktionen"
    //% block="ist Geste $geste erkannt"
    //% block.tooltip="Gibt WAHR zurück wenn die angegebene Geste erkannt wurde."
    //% geste.defl=GroveGesture.Right
    export function istGeste(geste: GroveGesture): boolean {
        return gestureLesen() === geste
    }

    /**
     * Hält das Programm an bis die angegebene Geste erkannt wird.
     */
    //% group="Erweiterungen"
    //% block="Warte auf Geste $geste"
    //% block.tooltip="Hält das Programm an bis die angegebene Geste erkannt wird."
    //% geste.defl=GroveGesture.Right
    export function wartAufGeste(geste: GroveGesture): void {
        while (gestureLesen() !== geste) {
            basic.pause(5)
        }
    }

    /**
     * Startet den Code einmalig wenn die Geste erkannt wird und deaktiviert die Geste.
     */
    //% group="Grundfunktionen"
    //% block="Starte einmalig, wenn Geste $geste erkannt"
    //% block.tooltip="Startet den Code einmalig wenn die Geste erkannt wird und deaktiviert die Geste."
    //% geste.defl=GroveGesture.Right
    export function onGestureOnce(geste: GroveGesture, handler: () => void): void {
        control.onEvent(gestureEventId, geste, function () {
            if (!triggeredGestures[geste]) {
                triggeredGestures[geste] = true
                handler()
            }
        })
        if (!monitoringLaeuft) {
            startGestureMonitoring()
        }
    }

    /**
     * Gibt die zuletzt erkannte Geste zurück ohne neu zu messen.
     */
    //% group="Erweiterungen"
    //% block="Letzte Geste"
    //% block.tooltip="Gibt die zuletzt erkannte Geste zurück ohne neu zu messen."
    export function letzteGesteAbrufen(): number {
        return letzteGeste
    }

    /**
     * Reaktiviert eine bestimmten Geste, sodass sie wieder ausgelöst werden kann.
     */
    //% group="Erweiterungen"
    //% block="Reaktiviere die Geste $geste"
    //% block.tooltip="Reaktiviert eine bestimmten Geste, sodass sie wieder ausgelöst werden kann"
    //% geste.defl=GroveGesture.Right
    export function resetGestureTrigger(geste: GroveGesture): void {
        triggeredGestures[geste] = false
    }

    /**
     * Reaktiviere alle Gesten, sodass sie wieder ausgelöst werden können.
     */
    //% group="Erweiterungen"
    //% block="Reaktiviere alle Gesten"
    //% block.tooltip="Reaktiviere alle Gesten, sodass sie wieder ausgelöst werden können"
    export function resetAllGestureTriggers(): void {
        for (let i = 0; i < triggeredGestures.length; i++) {
            triggeredGestures[i] = false
        }
    }

    /**
     * Gibt "WAHR" zurück wenn der Sensor korrekt initialisiert wurde.
     */
    //% group="Erweiterungen"
    //% block="Sensor erfolgreich initialisiert?"
    //% block.tooltip="Gibt WAHR zurück wenn der Sensor korrekt initialisiert wurde."
    export function istInitialisiert(): boolean {
        return sensorInitialisiert
    }

    /**
     * Auswahlfeld für Gesten (runder Block)
     */
    //% group="Erweiterungen"
    //% block="$geste"
    //% blockId="gesture_picker"
    //% geste.fieldEditor="gridpicker"
    //% geste.fieldOptions.columns=3
    //% geste.fieldOptions.tooltips=false
    //% geste.fieldOptions.width=200
    //% geste.defl=GroveGesture.Right
    export function auswahlGeste(geste: GroveGesture): GroveGesture {
        return geste
    }
}
